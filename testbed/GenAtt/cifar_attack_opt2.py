...
INIT_R = 10
MAX_R = 100
R_SEARCH = 10  # how many iteration to find a suitable R
MAX_ITER = 1000
EARLY_ABORT = True
...
class AntiDef:
    def __init__(self, session, data, model):
        ...
        self.R_PH = tf.placeholder(np.float32, (B_SIZE))

        # variables
        self.timg = tf.Variable(np.zeros(self.T_SHAPE), dtype=np.float32)
        self.R = tf.Variable(np.zeros((B_SIZE)), dtype=np.float32)
        ...
        loss2 = tf.maximum(0., (self.R - self.r2rodist))

        # calculate loss function
        self.loss = loss1 + 10*loss2 # using large panelty to prevent search within R
        ...
        self.input_v.append(self.R.assign(self.R_PH))
    ...
    def launch(self):
        ...
        for i in range(0, min(X.shape[0], B_SIZE*GEN_NUM), B_SIZE):
            ...
            # setup R
            batch_R = np.ones(B_SIZE) * INIT_R
            R_lb = np.zeros(B_SIZE)
            R_ub = np.ones(B_SIZE) * MAX_R
            ...
            # search R
            for outer in range(R_SEARCH):
                # reset
                self.sess.run(self.init)

                # print out R
                print('======== Current R is %f ========' % batch_R)
                ...
                # update R based on evaluation
                for e in range(B_SIZE):
                    if findatt[e]:
                        # find adversarial example, current R is upper bound
                        R_ub[e] = batch_R[e]
                        batch_R[e] = (R_lb[e] + R_ub[e]) / 2.
                    else:
                        # doesn't find adversarial example, current R is lower bound
                        R_lb[e] = batch_R[e]
                        batch_R[e] = (R_lb[e] + R_ub[e]) / 2.
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        # save result
        np.save(('results/att-%s-opt2.npy' % sys.argv[1]), np.array(att))
